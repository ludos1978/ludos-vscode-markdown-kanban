...
- Overlay Editor
  - in markdown mode
  - dual pane markdown more, with realtime preview and some editing modes
  - wysiwyg editor with buttons to add special features.

### Content Editor
- can add vscode standard shortcuts to insert snipplets (using special keys such as meta, ctrl, alt or shift)
- can use undo/redo
- All embed/link types (Link, Image, Include and Wiki) have a burger menu which allows mutliple actions. Alt+Click opens in the System Associated Editor.
  - Open (the file in an editor)
  - Open (in vscode if available, othervise in the System defined Tool)
  - Reveal in File Explorer
  - Search for Replacement File
  - Browse for Replacement File
  - Convert to Relative or Absolute Path
  - Delete
- All dynamic diagrams have a burger menu with mutiple actions.
  - Convert to SVG
...
### WYSIWYG Editor
- if a text is selected and the link-button or the image-button is clicked, we detect the type of the text that is selected (filename/path or normal text) and put it in the right part of the link. also make sure the selection stays the same.
- if a text is selected and any of the styling buttons are pressed, style this part of the text with the style. make sure the selection stays the same.
- a link/image must be editable, we use the burger menu function on it to modify it.
...
- it must be possible to put new content inside, before and after all blocks (and on the same line). putting conrent into an object differs for each type.
  ...
  - columns can be modified in the number of columns (-/+ in the top corners). But it's not possible to put text in front or after them! (the ---: :--: :--- must be on separate newlines!)
