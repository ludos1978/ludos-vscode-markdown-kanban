...
const { isWhiteSpace } = require("markdown-it/lib/common/utils.mjs");
 ...
 */
function parseMediaType(src, start, end) {
 ...
 */
function parseMediaSource(src, start, end, md = markdownIt()) {
  let pos = start;
 ...
 * @returns {null | { pos: number, value: string }}
 */
function parseNumber(src, start, end) {
  let pos = start;
 ...
 */
function parseMediaSize(src, start, end) {
  let pos = start;
 ...
 * @returns {null | { pos: number, value: string }}
 */
function parseMediaPoster(src, start, end, md = markdownIt()) {
  let pos = start;
  // poster must be ![](  <url>  )
  if (src.slice(pos, pos + 2) !== "#=") {
    return null;
 ...
 * @returns {null | { pos: number, value: string }}
 */
function parseLanguageTag(src, start, end) {
 ...
 */
function parseMediaCaptions(src, start, end, md = markdownIt()) {
  let pos = start;
 ...
 */
function parseMediaArgs(state, start) {
  let pos = start;
