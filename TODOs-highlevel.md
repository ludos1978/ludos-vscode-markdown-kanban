...
- when an image is dropped into vscode it can read and display it. but when i drop it into the kanban it can only create a link without the file path. would it somehow be possible to copy the file, suggest to the user to create a duplicate or similar so we have better external image handling?
