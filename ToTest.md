...
- [ ] In WYSIWYG, mermaid/plantuml blocks show clear ```lang ... ``` boundaries while editing, the burger menu includes the Edit action (no standalone Edit button), and edit mode shows split columns (code left, preview right) inside the diagram block with borders.
